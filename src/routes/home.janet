...
(defn code []
  [:div {:class "mw6"}
   [:span {:class "washed-purple"} "("] [:span {:class "orange"} "import"] [:span " "] [:span {:class "white"} "joy"] [:span {:class "washed-purple"} ")"]
   [:br]
   [:br]
   ...
   [:span " "]
   [:span {:class "white"} "routes"]
   [:span " "]
   [:span {:class "washed-purple"} "("]
   [:span {:class "white"} "joy/routes"]
   [:span " "]
   [:span {:class "washed-purple"} "["]
...
(defn features []
  [:hstack {:spacing "l" :class "responsive"}
   ...
   [:div {:class "feature-3"}
    (h1 "Clojure-ish syntax")
    [:p {:class "lh-copy black-ns white"} "Joy is written with janet, a modern lisp with a clojure-ish syntax that's something to be joyful about"]]])
